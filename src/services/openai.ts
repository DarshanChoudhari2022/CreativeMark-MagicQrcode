...
export async function generateReviewSuggestions(
  ...
  const prompt = `Generate 3 authentic customer review suggestions for a ${rating}-star Google review of "${businessName}"${businessContext} in ${languageMap[language] || 'English'}. Make them natural, specific, and varied in length (short, medium, detailed). Return as JSON array with format: [{"text": "review text"}]`;
